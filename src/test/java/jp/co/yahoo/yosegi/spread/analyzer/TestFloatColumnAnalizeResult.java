...
public class TestFloatColumnAnalizeResult {

  @Test
  public void T_getAnalizer_1() throws IOException{
    FloatColumnAnalizeResult result = new FloatColumnAnalizeResult( "name" , 100 , true , 10 , 90 , 2 , (float)40 , (float)50 );
    assertEquals( "name" , result.getColumnName() );
    assertEquals( ColumnType.FLOAT , result.getColumnType() );
    ...
    assertEquals( Float.BYTES * 90 , result.getLogicalDataSize() );
    assertEquals( (float)40 , result.getMin() );
    assertEquals( (float)50 , result.getMax() );
  }
