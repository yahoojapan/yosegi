...
public class LongNumEncoder implements INumEncoder {

  private final NumberToBinaryUtils.ILongConverter converter;

  public LongNumEncoder( final long min , final long max ) {
  ...
  public void toBinary(
    ...
    IWriteSupporter wrapBuffer =
        converter.toWriteSuppoter( rows , buffer , start , calcBinarySize( rows ) );
    for ( int i = 0 ; i < rows ; i++ ) {
      wrapBuffer.putLong( longArray[i].longValue() );
    }
  }
  ...
  public void setDictionary(
    ...
    IReadSupporter wrapBuffer =
        converter.toReadSupporter( buffer , start , calcBinarySize( rows ) );
    for ( int i = 0 ; i < rows ; i++ ) {
      dic.setLong( i , wrapBuffer.getLong() );
    }
  }
  ...
  public void loadInMemoryStorage(
    ...
    for ( int i = 0 ; i < isNullArray.length ; i++ ) {
      if ( isNullArray[i] ) {
        ...
        allocator.setLong( i + startIndex , wrapBuffer.getLong() );
      }
    }
  }
  ...
  public void setSequentialLoader(
    ...
    for (int i = 0; i < isNullArray.length; i++, index++) {
      if (isNullArray[i]) {
        ...
        loader.setLong(index, wrapBuffer.getLong());
      }
    }
