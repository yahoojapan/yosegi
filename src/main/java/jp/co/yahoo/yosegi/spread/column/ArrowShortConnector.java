...
public class ArrowShortConnector implements IArrowPrimitiveConnector {

  private final String columnName;
  private final SmallIntVector vector;

  public ArrowShortConnector( final String columnName , final SmallIntVector vector ) {
  ...
  public PrimitiveObject[] getPrimitiveObjectArray(
    ...
    for ( int i = start ; i < ( start + length ) && i < size() ; i++ ) {
      if ( ! vector.isNull( i ) ) {
        result[i - start] =  new ShortObj( vector.get( i ) );
      }
    }
    return result;
  }
  ...
  public void setPrimitiveObjectArray(
    ...
    for ( int i = start ; i < ( start + length ) && i < size() ; i++ ) {
      if ( vector.isNull( i ) ) {
        ...
        try {
          allocator.setPrimitiveObject( i - start , new ShortObj( vector.get( i ) ) );
