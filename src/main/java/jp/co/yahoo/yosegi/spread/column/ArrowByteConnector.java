...
public class ArrowByteConnector implements IArrowPrimitiveConnector {

  private final String columnName;
  private final TinyIntVector vector;

  public ArrowByteConnector( final String columnName , final TinyIntVector vector ) {
  ...
  public PrimitiveObject[] getPrimitiveObjectArray(
    ...
    for ( int i = start ; i < ( start + length ) && i < size() ; i++ ) {
      if ( ! vector.isNull( i ) ) {
        result[i - start] =  new ByteObj( vector.get( i ) );
      }
    }
    return result;
  }
  ...
  public void setPrimitiveObjectArray(
    ...
    for ( int i = start ; i < ( start + length ) && i < size() ; i++ ) {
      if ( vector.isNull( i ) ) {
        ...
        try {
          allocator.setPrimitiveObject( i - start , new ByteObj( vector.get( i ) ) );
