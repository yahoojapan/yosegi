...
public class ArrowFloatConnector implements IArrowPrimitiveConnector {

  private final String columnName;
  private final Float4Vector vector;

  public ArrowFloatConnector( final String columnName , final Float4Vector vector ) {
  ...
  public PrimitiveObject[] getPrimitiveObjectArray(
    ...
    for ( int i = start ; i < ( start + length ) && i < size() ; i++ ) {
      if ( ! vector.isNull( i ) ) {
        result[i - start] =  new FloatObj( vector.get( i ) );
      }
    }
    return result;
  }
  ...
  public void setPrimitiveObjectArray(
    ...
    for ( int i = start ; i < ( start + length ) && i < size() ; i++ ) {
      if ( vector.isNull( i ) ) {
        ...
        try {
          allocator.setPrimitiveObject( i - start , new FloatObj( vector.get( i ) ) );
