...
public class ArrowLongConnector implements IArrowPrimitiveConnector {

  private final String columnName;
  private final BigIntVector vector;

  public ArrowLongConnector( final String columnName , final BigIntVector vector ) {
  ...
  public PrimitiveObject[] getPrimitiveObjectArray(
    ...
    for ( int i = start ; i < ( start + length ) && i < size() ; i++ ) {
      if ( ! vector.isNull( i ) ) {
        result[i - start] =  new LongObj( vector.get( i ) );
      }
    }
    return result;
  }
  ...
  public void setPrimitiveObjectArray(
    ...
    for ( int i = start ; i < ( start + length ) && i < size() ; i++ ) {
      if ( vector.isNull( i ) ) {
        ...
        try {
          allocator.setPrimitiveObject( i - start , new LongObj( vector.get( i ) ) );
